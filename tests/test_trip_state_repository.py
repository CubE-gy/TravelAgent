...
def test_trip_state_can_be_saved_read_and_replaced_in_postgresql() -> None:
    ...
    try:
        with Session(engine) as session:
            ...
            assert save_trip_state(session, first_state) == first_state
        ...
        with Session(engine) as session:
            ...
            assert save_trip_state(session, replacement_state) == replacement_state

        with Session(engine) as session:
            assert get_trip_state(session, trip_id) == replacement_state
    ...
    try:
        with Session(engine) as session:
            with pytest.raises(ValueError, match="Trip not found"):
                save_trip_state(session, TripState(trip_id=uuid4()))
    finally:
        engine.dispose()
